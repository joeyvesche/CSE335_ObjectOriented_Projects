...
void CityView::OnLeftDoubleClick(wxMouseEvent &event)
{
    ...
    DoubleClickVisitor visitor; // double-click on a building
    DoubleClickStationVisitor svisitor; // double-click on the FireStation
    BurningBuildingCounter rvisitor;
    ResetBuilding rbvisitor;

    if (mGrabbedItem != nullptr) {
        mGrabbedItem->Accept(&svisitor);

        mGrabbedItem->Accept(&visitor);
        mGrabbedItem->Accept(&rvisitor);

        if (rvisitor.IfBurning())
        {
            mGrabbedItem->Accept(&rbvisitor);
        }
...
void CityView::OnAddTileMenuOption(wxCommandEvent& event)
{
    ...
    switch(event.GetId())
    {
        ...
        case IDM_BUILDINGS_FIRESTATION:
            tile = make_shared<TileFireStation>(&mCity);
            break;
