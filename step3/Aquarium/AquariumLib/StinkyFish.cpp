 ...
 */
StinkyFish::StinkyFish(Aquarium *aquarium) : Item(aquarium)
{
    mFishImage = make_unique<wxImage>(StinkyFishImageName, wxBITMAP_TYPE_ANY);
...
* @return true if hit.
*/
bool StinkyFish::HitTest(int x, int y)
{
    double wid = mFishBitmap->GetWidth();
    double hit = mFishBitmap->GetHeight();
